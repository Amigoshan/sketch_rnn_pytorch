...
class SketchRnn(nn.Module):
    ...
    def forward(self, x, seq_len, testing=False, use_gt=False):
        ...
        _, (hn, cn) = self.encoder(x_pack, self.init_hidden(batchNum)) # hn: 1 x batch x hidden
        ...
        if not testing:
            ...
            decoderInput = x[0:-1,:,:]
            decoderInput = torch.cat((decoderInput, sample.expand((seqNum, batchNum, self.hiddenNum))), dim=2)
